...
- The Signal Pipeline dashboard renders selected configured markets' session metadata and tradability constraints from `/api/signals/markets` in a Market Session feed, making timezone, session hours, trading days, tick size, lot size, fee/funding rates, contract multiplier, short support, leverage support, and max leverage visible outside raw API responses.
...
- `load_data_connector_registry_json()` can build configured CSV and SQLite connector registries from project JSON, resolving relative file and database paths against a caller-provided project root.
...
- `fetch_derivatives_with_cache()` now provides the matching generic DataConnector-to-DerivativeStore read/write boundary keyed by `DerivativeSeriesId`; the BTC derivative remote-fetch path uses this platform helper after the existing Parquet and pickle read fallbacks are checked.
...
- Generic research preview bar loading now goes through `fetch_bars_with_cache()` with `ParquetBarStore(data/research_bars)`, so configured non-BTC sources use the same BarStore-first connector cache path before feature, signal, risk, portfolio, and delivery layers run.
...
- Generic research previews now run the default non-BTC feature builder through `run_feature_engine_with_cache()` and `ParquetFeatureStore(data/research_features)`, giving configured research markets a FeatureStore boundary before standardized signals are generated.
- Generic swap/futures research feature runs now look for configured `data_type: "derivatives"` routes, load funding/open-interest through `fetch_derivatives_with_cache()` and `ParquetDerivativeStore(data/research_derivatives)`, and append `DerivativesFeatureModule` output with derivative-enabled FeatureStore keys kept separate from OHLCV-only cache keys.
...
- Generic research previews now default to a cached `FeatureEngine` composed from technical indicators, Donchian channels, volume z-score, ATR/ADX, Bollinger bands, and price-action features; the trend EMA, ATR, ADX, Bollinger period, and Bollinger standard-deviation multiplier come from the resolved `RegimeProfile`, and those parameters are included in the persisted feature set name, so non-BTC markets do not inherit BTC-only regime-feature assumptions or collide with stale caches from other profiles.
...
- Generic research feature runs can now load configured order-book snapshot routes through `fetch_order_book_snapshots_with_cache()` and `ParquetOrderBookStore(data/research_order_books)`, append `OrderBookFeatureModule`, and include the configured depth in the FeatureStore key.
...
- `RiskEngine` now evaluates portfolio, symbol, module, and correlation-group budget gates with the candidate's actual capped `risk_amount` after notional, leverage, and contract-multiplier sizing, rather than the larger pre-cap target risk.
...
- `RiskEngine.budget_diagnostics()` now returns portfolio, symbol, module, and correlation-group budget usage, remaining budget, utilization, target next-trade risk, and current loss-streak pause state without changing risk gate behavior.
...
- Event-backtest REST routes can now pass explicit execution-simulation query parameters into `BacktestExecutionConfig`, including intrabar entry/exit flags, fee/slippage settings, entry/exit partial-fill caps, entry/exit volume-participation caps, stale entry/exit order age, and optional entry/exit spread feature names.
...
- The Signal Pipeline dashboard now renders the serialized event exposure curve as an Event Exposure feed, includes terminal order count in the event summary cards, and exposes entry-limit / stale-entry-age / stale-exit-age / exit-fill-cap / exit-volume-cap / entry-spread-feature / exit-spread-feature controls for event-backtest requests, making recent position count, long/short/gross/net notional, open risk, stale-order cancellations, and spread-adjusted entry/exit research visible next to event equity and trades.
...
- The Signal Pipeline dashboard now renders `riskDiagnostics` as portfolio, symbol, module, and correlation-group budget usage rows.
...
- Generic research previews now default to `load_research_preview_bars()`, which keeps the BTC cached-bar fallback but loads configured non-BTC bars through `DataConnectorRegistry` and `config/research_data_sources.json`.
