...
- The Signal Pipeline dashboard now renders configured market session metadata and tradability constraints in a Market Session feed, including base/quote asset identity, session hours, trading days, tick size, lot size, fee/funding rates, contract multiplier, short support, leverage support, and max leverage when present.
...
- `load_data_connector_registry_json()` can pass `column_map` records into CSV/SQLite connectors, so vendor-specific OHLCV and turnover column names can be normalized from configuration instead of code changes.
...
- `CcxtExchangeConnector` can pass configured OHLCV `start`/`end` windows to CCXT bar fetches through `since` and best-effort `endTime`, then filter normalized bars locally before they enter the BarStore-backed research path.
...
- Local CSV and SQLite bar connectors now preserve optional `turnover` and common quote-volume source columns as standardized `Turnover`, so configured research data sources can carry traded-value bars through the DataConnector-to-BarStore boundary instead of reducing bars to OHLCV only.
- Local CSV and SQLite bar connectors now accept explicit source-to-standard `column_map` configuration for Open, High, Low, Close, Volume, and Turnover fields, keeping local vendor schemas outside strategy code.
...
- Generic research preview bar loading now uses `fetch_bars_with_cache()` with `ParquetBarStore(data/research_bars)` by default or `SQLiteBarStore(data/research_bars)` when a route sets `store_type: "sqlite"`, so configured non-BTC sources can share the same BarStore-first cache boundary as the platform connector layer.
- Generic research data routes can now pass configured `start` and `end` bar windows into the DataConnector-to-BarStore cache boundary, and configured derivative windows into the DataConnector-to-DerivativeStore cache boundary, with naive date strings treated as UTC.
...
- Generic research previews now default to a cached `FeatureEngine` run for non-BTC markets, loading the base technical, Donchian, volume, volatility, Bollinger, and price-action feature module set from `config/research_feature_modules.json` before writing through `ParquetFeatureStore(data/research_features)` by default or `SQLiteFeatureStore(data/research_features)` when research data config sets `feature_store_type: "sqlite"` before signal, risk, portfolio, and delivery layers run.
- `VolumeFeatureModule` now derives rolling traded-value features from standardized `Turnover` when available, adding `turnover_sma_*`, `turnover_std_*`, and `turnover_zscore_*` beside volume normalization without requiring signal modules to inspect raw bar storage.
- Generic research FeatureStore keys now include a `turnover` schema marker when input bars carry standardized `Turnover`, so traded-value feature runs cannot reuse stale OHLCV-only caches.
- Generic swap/futures research feature runs can now load configured funding/open-interest data through the generic `DataConnector` to `DerivativeStore` cache boundary, using `ParquetDerivativeStore(data/research_derivatives)` by default or `SQLiteDerivativeStore(data/research_derivatives)` when a route sets `store_type: "sqlite"`, including route-level `start`/`end` windows, append `DerivativesFeatureModule` output, and isolate derivative-enabled FeatureStore keys from OHLCV-only caches.
...
- Generic research feature runs can now load configured order-book snapshot routes through the generic `DataConnector` to `OrderBookStore` cache boundary, using `ParquetOrderBookStore(data/research_order_books)` by default or `SQLiteOrderBookStore(data/research_order_books)` when a route sets `store_type: "sqlite"`, append `OrderBookFeatureModule` output, and isolate depth-enabled FeatureStore keys from OHLCV-only caches.
- Generic research feature runs can now load one or more configured `data_type: "external_metrics"` routes from `ParquetExternalMetricStore(data/research_external_metrics)` by default or `SQLiteExternalMetricStore(data/research_external_metrics)` when a route sets `store_type: "sqlite"`, apply route-level `start`/`end` windows, append `ExternalMetricFeatureModule` output with route-configured prefixes and columns, and isolate external-metric-enabled FeatureStore keys from OHLCV-only caches.
...
- `config/research_feature_modules.json` now defines the default generic research feature module set, including `$regime.*` placeholders so trend EMA, ATR, ADX, and Bollinger parameters still resolve from each market's `RegimeProfile` instead of being hardcoded in the preview service.
...
- Generic research FeatureStore keys now include the resolved `RegimeProfile` default feature parameters for trend EMA, ATR, ADX, Bollinger bands, and bar-schema enrichments such as `Turnover`, so asset/profile-specific or traded-value feature outputs cannot collide under the same persisted `research_default_v1` feature set.
...
- `RiskBudgetDiagnostics` now reports portfolio, symbol, module, correlation-group, exchange, and market-type risk usage from `RiskEngine`, and `SignalPipeline` / preview payloads expose the same snapshot after portfolio planning.
- The Signal Pipeline dashboard now renders `riskDiagnostics` as a risk-budget table, making portfolio, symbol, module, correlation-group, exchange, and market-type risk usage visible in the frontend.
- `RiskLimits.max_exchange_risk` and `RiskLimits.max_market_type_risk` now add default-off portfolio risk gates for exchange and market-type concentration, with `SignalPipeline` passing existing-position and same-batch open risk from `MarketSpec.exchange` and `MarketSpec.market_type`.
- Generic research preview and research event-backtest service/API callers can now pass risk-limit overrides such as `max_exchange_risk` and `max_market_type_risk`, so configured market metadata can drive enforceable research risk budgets from the preview boundary instead of only from direct Python construction.
- The Signal Pipeline dashboard now exposes research risk-limit controls for generic preview/event-backtest requests, passing `risk_per_trade`, `portfolio_risk_budget`, symbol/module/correlation/exchange/market-type risk caps, and max drawdown through the same API boundary.
...
- `RiskEngine` now checks portfolio, symbol, module, correlation-group, exchange, and market-type risk budgets against the candidate's actual capped `risk_amount` after notional, leverage, and contract constraints are applied, so constrained markets are not rejected by a larger pre-cap target risk.
...
- `EventDrivenBacktestResult.order_latency` and `order_latency_summary` now record submitted-order wait time in bars from first submitted/partial state to filled, canceled, or rejected resolution; `BacktestOrderLatency` and `BacktestOrderLatencySummary` are exported from the platform package, BTC/generic event-preview payloads serialize `orderLatency` and `orderLatencySummary`, and the dashboard summary shows average and maximum order wait.
- The Signal Pipeline dashboard now renders an Event Order Latency table from `orderLatency`, showing each resolved order's id, status, symbol, layer, module, submitted bar, resolved bar, and wait bars without requiring raw JSON inspection.
- `EventDrivenBacktestResult.open_order_ages` and `open_order_age_summary` now capture still-open submitted/partially-filled order ages at run end; `BacktestOpenOrderAge` and `BacktestOpenOrderAgeSummary` are exported from the platform package, BTC/generic event-preview payloads serialize `openOrderAges` and `openOrderAgeSummary`, and the dashboard Event Order Latency table plus event summary cards include unresolved order count, age rows, average age, and max age beside resolved latency rows.
- `EventDrivenBacktestResult.order_lifecycle_summary` now derives total, filled, open, resolved, and terminal order counts plus fill/open/terminal rates from effective event orders; `order_lifecycle_by_action`, `order_lifecycle_by_module`, `order_lifecycle_by_symbol`, `order_lifecycle_by_layer`, `order_lifecycle_by_direction`, `order_lifecycle_by_correlation_group`, `order_lifecycle_by_exchange`, and `order_lifecycle_by_market_type` apply the same lifecycle summary per order action, `SignalModule`, symbol, portfolio layer, direction, `MarketSpec.correlation_group`, exchange, and `MarketSpec.market_type`, BTC/generic event-preview payloads serialize `orderLifecycleSummary`, `orderLifecycleByAction`, `orderLifecycleByModule`, `orderLifecycleBySymbol`, `orderLifecycleByLayer`, `orderLifecycleByDirection`, `orderLifecycleByCorrelationGroup`, `orderLifecycleByExchange`, and `orderLifecycleByMarketType`, and the dashboard event summary cards plus Event Action/Module/Symbol/Layer/Direction/Correlation/Exchange/Market Type Lifecycle tables expose those execution-quality rates beside raw order counts.
...
- Event-backtest REST routes can now accept execution-simulation query parameters such as `intrabar_entry_limit=true`, `intrabar_stop_target=true`, `fee_rate=...`, `slippage_bps=...`, `max_entry_order_age_bars=...`, `max_exit_order_age_bars=...`, `max_entry_fill_fraction_per_bar=...`, `max_entry_volume_fraction_per_bar=...`, `max_exit_fill_fraction_per_bar=...`, `max_exit_volume_fraction_per_bar=...`, `entry_spread_feature=order_book_spread`, and `exit_spread_feature=order_book_spread`, passing them into `BacktestExecutionConfig` for research previews without changing default compatibility behavior.
- The Signal Pipeline dashboard now renders `terminalOrderCount` in the event-backtest summary cards and exposes controls for entry-limit fills, stop/target High-Low triggers, fee/slippage assumptions, stale entry/exit order age, entry/exit fill and volume caps, and optional entry/exit spread feature names, making stale-order cancellation, partial-fill/participation, stop/target trigger, cost, and order-book-spread execution research available without editing query strings.
...
- The BTC/generic event-driven preview APIs now serialize each `equityCurve` point with event return, current equity peak, drawdown amount, drawdown percent, and drawdown duration bars, so dashboard/API callers can inspect per-event return and underwater state beside cash, unrealized PnL, and total equity.
